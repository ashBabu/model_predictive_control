...
class MayaviRendering:
    ...
    def anim(self, rs=None, angs=None, q=None, fig_save=False):
        ...
        if fig_save:
            mlab.savefig("/home/ash/Ash/repo/model_predictive_control/src/animation/%02d.png" % 0)
        if angs.shape[1] > 3:
            n = angs.shape[1]
            for i in range(1, n):
                ...
                if fig_save:
                    mlab.savefig("/home/ash/Ash/repo/model_predictive_control/src/animation/%02d.png" % i)
                yield
...
if __name__ == '__main__':
    ...
    t = np.load('t.npy', allow_pickle=True)
    r_s, ang_s, q, q_dot, pv_com = np.load('rs.npy', allow_pickle=True), np.load('ang_s.npy', allow_pickle=True),\
                                       np.load('q.npy', allow_pickle=True), np.load('q_dot.npy', allow_pickle=True),\
                                                                               np.load('pv_com.npy', allow_pickle=True)
    if i == 1:
